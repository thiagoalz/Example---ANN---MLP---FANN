...
int main()
{
    ...
    struct fann *ann = fann_create_from_file("mlp1.net");//carregando rede
    ...
    infile.open ("run.data", ifstream::in);
   ...
   while( (!infile.eof()) && (testeAtual<QtdTestes) ) {
	...
	for(int j=0; j<QtdSaidas;j++){
		sprintf(buff,"%3.1f", calc_out[j]);
		strSaida=strSaida=strSaida+" "+buff;
	}
...
//===========================
  struct fann_train_data *data = fann_read_train_from_file("run.data");
  fann_reset_MSE(ann);
